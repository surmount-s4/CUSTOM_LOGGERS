...
$Global:ServiceConfig = @{
    ...
    MainLogFile = 'SecurityEvents.csv'
    CorrelationLogFile = 'CorrelationEvents.csv'
    USBLogFile = 'USBThreatEvents.csv'
    OTLogFile = 'OTProcessEvents.csv'
    ServiceLogFile = 'ServiceHealth.csv'
    ErrorLogFile = 'ErrorEvents.csv'
    MaxLogSize = 100MB
    LogRotationDays = 30
...
# CSV Headers for different log files
$Global:CSVHeaders = @{
    Main = 'Timestamp,Severity,MITRE_Technique,Detection_Module,Event_Details,Process_ID,Process_Name,Command_Line,User,Source_IP,Dest_IP,File_Path,Registry_Key,Additional_Context'
    Correlation = 'Timestamp,Correlation_Type,Confidence,Attack_Chain,Related_Event_IDs,Duration,Risk_Score,Kill_Chain_Phase'
    USB = 'Timestamp,Severity,Device_Type,Device_Name,Device_ID,Threat_Level,MITRE_Technique,Action,User,Additional_Info'
    OT = 'Timestamp,Severity,Process_Name,Parent_Process,Command_Line,Process_ID,User,Whitelist_Status,OT_Risk_Level,Additional_Context'
    Service = 'Timestamp,Component,Status,CPU_Usage,Memory_Usage,Event_Count,Error_Count,Additional_Info'
    Error = 'Timestamp,Module,Error_Type,Error_Message,Stack_Trace,Recovery_Action'
}
...
function Initialize-LoggingSystem {
    try {
        ...
        foreach ($logType in $Global:CSVHeaders.Keys) {
            $logFile = switch ($logType) {
                'Main' { Join-Path $Global:ServiceConfig.LogBasePath $Global:ServiceConfig.MainLogFile }
                'Correlation' { Join-Path $Global:ServiceConfig.LogBasePath $Global:ServiceConfig.CorrelationLogFile }
                'USB' { Join-Path $Global:ServiceConfig.LogBasePath $Global:ServiceConfig.USBLogFile }
                'OT' { Join-Path $Global:ServiceConfig.LogBasePath $Global:ServiceConfig.OTLogFile }
                'Service' { Join-Path $Global:ServiceConfig.LogBasePath $Global:ServiceConfig.ServiceLogFile }
                'Error' { Join-Path $Global:ServiceConfig.LogBasePath $Global:ServiceConfig.ErrorLogFile }
            }
            
            # Create file with header if it doesn't exist
            if (!(Test-Path $logFile)) {
                $Global:CSVHeaders[$logType] | Out-File -FilePath $logFile -Encoding UTF8
            }
            
            # Initialize log writer
            $Global:LogWriters[$logType] = @{
                Path = $logFile
                LastWrite = Get-Date
                EventCount = 0
            }
        ...
        Write-ServiceLog -Message "Logging system initialized successfully" -Component "LoggingSystem" -Status "INFO"
    }
    catch {
    ...
    try {
        ...
        $csvRow = switch ($LogType) {
            ...
            'Correlation' {
                ...
                "$($EventData.AttackChain)," +
                "$($EventData.RelatedEventIDs)," +
                "$($EventData.Duration)," +
                "$($EventData.RiskScore)," +
            ...
            'USB' {
                ...
                "`"$($EventData.DeviceName -replace '"', '""')`"," +
                "$($EventData.DeviceID)," +
                "$($EventData.ThreatLevel)," +
                "$($EventData.MitreTechnique)," +
                "$($EventData.Action)," +
                "$($EventData.User)," +
            ...
            'OT' {
                ...
                "$($EventData.Severity)," +
                "`"$($EventData.ProcessName -replace '"', '""')`"," +
                "`"$($EventData.ParentProcess -replace '"', '""')`"," +
                "`"$($EventData.CommandLine -replace '"', '""')`"," +
                "$($EventData.ProcessID)," +
                "$($EventData.User)," +
                "$($EventData.WhitelistStatus)," +
            ...
            'Error' {
                ...
                "$($EventData.Module)," +
                "$($EventData.ErrorType)," +
                "`"$($EventData.ErrorMessage -replace '"', '""')`"," +
...
function Write-ServiceLog {
    ...
    $eventData = @{
        Component = $Component
        Status = $Status
        CPUUsage = (Get-Process -Id $PID).CPU
        MemoryUsage = [System.GC]::GetTotalMemory($false)
        EventCount = $Global:LogWriters['Main'].EventCount
        ErrorCount = $Global:LogWriters['Error'].EventCount
        AdditionalInfo = $Message
    }
    
    Write-SecurityEvent -LogType 'Service' -EventData $eventData
}
...
function Invoke-LogRotation {
    ...
    try {
        ...
        Write-ServiceLog -Message "Log rotation completed for $LogType" -Component "LogRotation" -Status "INFO"
    }
    catch {
        Write-ServiceLog -Message "Log rotation failed for $LogType`: $($_.Exception.Message)" -Component "LogRotation" -Status "ERROR"
    }
}
...
Log Files (CSV format):
  SecurityEvents.csv      - Main security detections
  CorrelationEvents.csv   - Attack chain correlations
  USBThreatEvents.csv     - USB device threats
  OTProcessEvents.csv     - OT process monitoring
  ServiceHealth.csv       - Service health metrics
  ErrorEvents.csv         - Error logs
