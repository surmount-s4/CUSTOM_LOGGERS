...
# MITRE ATT&CK Technique to Category Mapping
$Global:MitreMapping = @{
    'T1566.001' = @{ Category = 'InitialAccess'; Name = 'Spearphishing Attachment' }
    'T1566.002' = @{ Category = 'InitialAccess'; Name = 'Spearphishing Link' }
    'T1189' = @{ Category = 'InitialAccess'; Name = 'Drive-by Compromise' }
    'T1190' = @{ Category = 'InitialAccess'; Name = 'Exploit Public-Facing Application' }
    'T1133' = @{ Category = 'InitialAccess'; Name = 'External Remote Services' }
    'T1078' = @{ Category = 'InitialAccess'; Name = 'Valid Accounts' }
    'T1059' = @{ Category = 'Execution'; Name = 'Command and Scripting Interpreter' }
    'T1059.001' = @{ Category = 'Execution'; Name = 'PowerShell' }
    'T1059.003' = @{ Category = 'Execution'; Name = 'Windows Command Shell' }
    'T1059.005' = @{ Category = 'Execution'; Name = 'Visual Basic' }
    'T1059.007' = @{ Category = 'Execution'; Name = 'JavaScript' }
    'T1203' = @{ Category = 'Execution'; Name = 'Exploitation for Client Execution' }
    'T1204' = @{ Category = 'Execution'; Name = 'User Execution' }
    'T1204.001' = @{ Category = 'Execution'; Name = 'Malicious Link' }
    'T1204.002' = @{ Category = 'Execution'; Name = 'Malicious File' }
    'T1218' = @{ Category = 'Execution'; Name = 'System Binary Proxy Execution' }
    'T1218.005' = @{ Category = 'Execution'; Name = 'Mshta' }
    'T1218.010' = @{ Category = 'Execution'; Name = 'Regsvr32' }
    'T1218.011' = @{ Category = 'Execution'; Name = 'Rundll32' }
    'T1547' = @{ Category = 'Persistence'; Name = 'Boot or Logon Autostart Execution' }
    'T1547.001' = @{ Category = 'Persistence'; Name = 'Registry Run Keys / Startup Folder' }
    'T1053' = @{ Category = 'Persistence'; Name = 'Scheduled Task/Job' }
    'T1053.005' = @{ Category = 'Persistence'; Name = 'Scheduled Task' }
    'T1543.003' = @{ Category = 'Persistence'; Name = 'Windows Service' }
    'T1546' = @{ Category = 'Persistence'; Name = 'Event Triggered Execution' }
    'T1546.003' = @{ Category = 'Persistence'; Name = 'Windows Management Instrumentation Event Subscription' }
    'T1055' = @{ Category = 'DefenseEvasion'; Name = 'Process Injection' }
    'T1036' = @{ Category = 'DefenseEvasion'; Name = 'Masquerading' }
    'T1036.003' = @{ Category = 'DefenseEvasion'; Name = 'Rename System Utilities' }
    'T1112' = @{ Category = 'DefenseEvasion'; Name = 'Modify Registry' }
    'T1134' = @{ Category = 'DefenseEvasion'; Name = 'Access Token Manipulation' }
    'T1562.002' = @{ Category = 'DefenseEvasion'; Name = 'Disable Windows Event Logging' }
    'T1564' = @{ Category = 'DefenseEvasion'; Name = 'Hide Artifacts' }
    'T1070.004' = @{ Category = 'DefenseEvasion'; Name = 'File Deletion' }
    'T1087' = @{ Category = 'Discovery'; Name = 'Account Discovery' }
    'T1087.001' = @{ Category = 'Discovery'; Name = 'Local Account' }
    'T1087.002' = @{ Category = 'Discovery'; Name = 'Domain Account' }
    'T1012' = @{ Category = 'Discovery'; Name = 'Query Registry' }
    'T1046' = @{ Category = 'Discovery'; Name = 'Network Service Scanning' }
    'T1082' = @{ Category = 'Discovery'; Name = 'System Information Discovery' }
    'T1083' = @{ Category = 'Discovery'; Name = 'File and Directory Discovery' }
    'T1217' = @{ Category = 'Discovery'; Name = 'Browser Bookmark Discovery' }
    'T1091' = @{ Category = 'InitialAccess'; Name = 'Replication Through Removable Media' }
    'T1052' = @{ Category = 'Exfiltration'; Name = 'Exfiltration Over Physical Medium' }
    'T1027' = @{ Category = 'DefenseEvasion'; Name = 'Obfuscated Files or Information' }
}
...
$Global:UnifiedCSVHeader = 'Timestamp,Event_Type,Severity,Log_Source,MITRE_Techniques,MITRE_Categories,Event_Details,Process_ID,Process_Name,Parent_Process,Command_Line,User,Source_IP,Dest_IP,File_Path,Registry_Key,Device_Info,Service_Info,Error_Info,Risk_Score,Additional_Context'
...
function Get-PossibleMitreTechniques {
    param(
        [string]$ProcessName,
        [string]$CommandLine,
    ...
    # Email client spawning processes
    if ($ParentProcess -match "outlook\.exe|thunderbird\.exe|mailbird\.exe") {
        $possibleTechniques += @('T1566.001', 'T1566.002', 'T1204.002')
    }
    
    # Office applications spawning processes
    if ($ParentProcess -match "winword\.exe|excel\.exe|powerpnt\.exe") {
        $possibleTechniques += @('T1566.001', 'T1203', 'T1204.002')
    }
    
    # Browser spawning processes
    if ($ParentProcess -match "chrome\.exe|firefox\.exe|msedge\.exe|iexplore\.exe") {
        $possibleTechniques += @('T1189', 'T1203', 'T1204.001')
    }
    
    # Web server spawning processes
    if ($ParentProcess -match "w3wp\.exe|httpd\.exe|nginx\.exe|apache\.exe|iisexpress\.exe") {
        $possibleTechniques += @('T1190')
    }
    
    # Script interpreters
    if ($ProcessName -match "powershell\.exe|pwsh\.exe") {
        $possibleTechniques += @('T1059', 'T1059.001')
    }
    if ($ProcessName -match "cmd\.exe") {
        $possibleTechniques += @('T1059', 'T1059.003')
    }
    if ($ProcessName -match "wscript\.exe|cscript\.exe") {
        $possibleTechniques += @('T1059', 'T1059.005', 'T1059.007')
    }
    
    # Suspicious command line arguments
    if ($CommandLine -match "-encodedcommand|-enc|-nop|-w hidden|bypass|iex|invoke-expression|downloadstring|frombase64string") {
        $possibleTechniques += @('T1059.001', 'T1027')
    }
    
    # Living-off-the-land binaries
    if ($ProcessName -match "mshta\.exe") {
        $possibleTechniques += @('T1218', 'T1218.005')
    }
    if ($ProcessName -match "regsvr32\.exe") {
        $possibleTechniques += @('T1218', 'T1218.010')
    }
    if ($ProcessName -match "rundll32\.exe") {
        $possibleTechniques += @('T1218', 'T1218.011')
    }
    if ($ProcessName -match "certutil\.exe|bitsadmin\.exe|wmic\.exe|msbuild\.exe") {
        $possibleTechniques += @('T1218')
    }
    
    # Registry autostart locations
    if ($RegistryKey -match "CurrentVersion\\\\Run|CurrentVersion\\\\RunOnce|Winlogon") {
        $possibleTechniques += @('T1547', 'T1547.001')
    }
    
    # Scheduled tasks
    if ($RegistryKey -match "Schedule\\\\TaskCache" -or $EventDetails -match "scheduled task") {
        $possibleTechniques += @('T1053', 'T1053.005')
    }
    
    # Service operations
    if ($RegistryKey -match "ControlSet\\\\Services\\\\" -or $EventDetails -match "service") {
        $possibleTechniques += @('T1543.003')
    }
    
    # WMI events
    if ($EventDetails -match "WMI|Windows Management Instrumentation") {
        $possibleTechniques += @('T1546.003')
    }
    
    # Process injection indicators
    if ($EventDetails -match "process injection|CreateRemoteThread|WriteProcessMemory") {
        $possibleTechniques += @('T1055')
    }
    
    # File paths indicating execution from temp/downloads
    if ($FilePath -match "\\\\Temp\\\\|\\\\Downloads\\\\|\\\\AppData\\\\Local\\\\Temp\\\\") {
        $possibleTechniques += @('T1204', 'T1204.002')
    }
    
    # Double extension files
    if ($FilePath -match "\\.(pdf|doc|docx|xls|xlsx|txt)\\.(exe|scr|com|bat)$") {
        $possibleTechniques += @('T1036', 'T1204.002')
    }
    
    # System binary masquerading
    if ($ProcessName -match "svchost\.exe|lsass\.exe|winlogon\.exe|csrss\.exe|smss\.exe" -and $FilePath -notmatch "\\\\System32\\\\|\\\\SysWOW64\\\\") {
        $possibleTechniques += @('T1036', 'T1036.003')
    }
    
    # Registry modification
    if ($LogSource -match "Registry" -or $RegistryKey) {
        $possibleTechniques += @('T1112')
    }
    
    # Account discovery commands
    if ($CommandLine -match "net user|net localgroup|whoami|query user") {
        $possibleTechniques += @('T1087', 'T1087.001', 'T1087.002')
    }
    
    # System information discovery
    if ($CommandLine -match "systeminfo|hostname|ipconfig|netstat") {
        $possibleTechniques += @('T1082')
    }
    
    # File/directory discovery
    if ($CommandLine -match "dir |ls |find |tree ") {
        $possibleTechniques += @('T1083')
    }
    
    # Network scanning
    if ($CommandLine -match "ping|telnet|nmap|portscan") {
        $possibleTechniques += @('T1046')
    }
    
    # USB/Device related
    if ($LogSource -eq "USB" -or $EventDetails -match "USB|device") {
        $possibleTechniques += @('T1091', 'T1052')
    }
    
    # Get unique techniques and their categories
    $uniqueTechniques = $possibleTechniques | Sort-Object | Get-Unique
    foreach ($technique in $uniqueTechniques) {
        if ($Global:MitreMapping.ContainsKey($technique)) {
            $possibleCategories += $Global:MitreMapping[$technique].Category
        }
    }
    $uniqueCategories = $possibleCategories | Sort-Object | Get-Unique
...
function Initialize-LoggingSystem {
    try {
        ...
        if (!(Test-Path $logFile)) {
            $Global:UnifiedCSVHeader | Out-File -FilePath $logFile -Encoding UTF8
        }
        ...
        Write-UnifiedLog -EventType "ServiceEvent" -Severity "INFO" -LogSource "LoggingSystem" -EventDetails "Logging system initialized successfully"
    }
    catch {
...
function Write-UnifiedLog {
    ...
    try {
        ...
        if (-not $SkipDeduplication) {
            $eventHash = "$LogSource-$ProcessID-$EventDetails"
        ...
        # Get possible MITRE techniques and categories
        $mitreAnalysis = Get-PossibleMitreTechniques -ProcessName $ProcessName -CommandLine $CommandLine -ParentProcess $ParentProcess -FilePath $FilePath -RegistryKey $RegistryKey -EventDetails $EventDetails -LogSource $LogSource

        $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss.fff"

        # Clean command line for CSV safety - exclude Python code/content
        $cleanCommandLine = if (
            ($CommandLine -match '(?i)python\.exe') -or 
            ($CommandLine -match '(?s)def\s|import\s|class\s|print\(')
        ) {
            ''
        } else {
            ConvertTo-CSVSafeString $CommandLine
        }
        ...
        $csvRow = @(
            ...
            $Severity,
            $LogSource,
            "`"$($mitreAnalysis.Techniques)`"",
            "`"$($mitreAnalysis.Categories)`"",
            "`"$(ConvertTo-CSVSafeString $EventDetails)`"",
            $ProcessID,
            "`"$(ConvertTo-CSVSafeString $ProcessName)`"",
            "`"$(ConvertTo-CSVSafeString $ParentProcess)`"",
            "`"$cleanCommandLine`"",
            $User,
            $SourceIP,
            $DestIP,
            "`"$(ConvertTo-CSVSafeString $FilePath)`"",
            "`"$(ConvertTo-CSVSafeString $RegistryKey)`"",
            "`"$(ConvertTo-CSVSafeString $DeviceInfo)`"",
            "`"$(ConvertTo-CSVSafeString $ServiceInfo)`"",
            "`"$(ConvertTo-CSVSafeString $ErrorInfo)`"",
            $RiskScore,
            "`"$(ConvertTo-CSVSafeString $AdditionalContext)`""
        ) -join ","

        # Write to file
        $logFile = $Global:LogWriter.Path
        $csvRow | Out-File -FilePath $logFile -Append -Encoding UTF8

        # Update statistics
        $Global:LogWriter.EventCount++
        $Global:LogWriter.LastWrite = Get-Date
        ...
        Write-Error "Failed to write unified log: $($_.Exception.Message)"
    }
}

# Legacy function for backward compatibility - redirects to unified logging
function Write-SecurityEvent {
    ...
    # Map old log types to new event types
    $eventType = switch ($LogType) {
        'Main' { 'SecurityEvent' }
    ...
    $params = @{
        EventType = $eventType
        Severity = if ($EventData.Severity) { $EventData.Severity } else { 'INFO' }
        LogSource = if ($EventData.DetectionModule) { $EventData.DetectionModule } else { $LogType }
        EventDetails = if ($EventData.EventDetails) { $EventData.EventDetails } else { 'No details' }
        ProcessID = if ($EventData.ProcessID) { $EventData.ProcessID } else { '' }
        ProcessName = if ($EventData.ProcessName) { $EventData.ProcessName } else { '' }
        ParentProcess = if ($EventData.ParentProcess) { $EventData.ParentProcess } else { '' }
        CommandLine = if ($EventData.CommandLine) { $EventData.CommandLine } else { '' }
        User = if ($EventData.User) { $EventData.User } else { '' }
        SourceIP = if ($EventData.SourceIP) { $EventData.SourceIP } else { '' }
        DestIP = if ($EventData.DestIP) { $EventData.DestIP } else { '' }
        FilePath = if ($EventData.FilePath) { $EventData.FilePath } else { '' }
        RegistryKey = if ($EventData.RegistryKey) { $EventData.RegistryKey } else { '' }
        AdditionalContext = if ($EventData.AdditionalContext) { $EventData.AdditionalContext } else { '' }
        SkipDeduplication = $SkipDeduplication
    }

    # Add specialized fields based on log type
    if ($LogType -eq 'USB') {
        $params.DeviceInfo = "Type:$($EventData.DeviceType);Name:$($EventData.DeviceName);ID:$($EventData.DeviceID);ThreatLevel:$($EventData.ThreatLevel);Action:$($EventData.Action)"
    }
    elseif ($LogType -eq 'Service') {
        $params.ServiceInfo = "Component:$($EventData.Component);Status:$($EventData.Status);CPU:$($EventData.CPUUsage);Memory:$($EventData.MemoryUsage);Events:$($EventData.EventCount);Errors:$($EventData.ErrorCount)"
    }
    elseif ($LogType -eq 'Error') {
        $params.ErrorInfo = "Module:$($EventData.Module);Type:$($EventData.ErrorType);Message:$($EventData.ErrorMessage);Recovery:$($EventData.RecoveryAction)"
    }
    elseif ($LogType -eq 'Correlation') {
        $params.AdditionalContext = "Type:$($EventData.CorrelationType);Confidence:$($EventData.Confidence);Chain:$($EventData.AttackChain);Events:$($EventData.RelatedEventIDs);Duration:$($EventData.Duration);Phase:$($EventData.KillChainPhase)"
        $params.RiskScore = if ($EventData.RiskScore) { $EventData.RiskScore } else { '' }
    }

    Write-UnifiedLog @params
}
...
function Write-ServiceLog {
    ...
    $serviceInfo = "Component:$Component;Status:$Status;CPU:$((Get-Process -Id $PID).CPU);Memory:$([System.GC]::GetTotalMemory($false));Events:$($Global:LogWriter.EventCount)"
    
    Write-UnifiedLog -EventType "ServiceEvent" -Severity $Status -LogSource "ServiceHealth" -EventDetails $Message -ServiceInfo $serviceInfo
}
...
function Invoke-LogRotation {
    try {
        ...
        Write-UnifiedLog -EventType "ServiceEvent" -Severity "INFO" -LogSource "LogRotation" -EventDetails "Log rotation completed" -AdditionalContext "BackupFile: $backupFile"
    }
    catch {
        Write-UnifiedLog -EventType "ErrorEvent" -Severity "ERROR" -LogSource "LogRotation" -EventDetails "Log rotation failed" -ErrorInfo "Error: $($_.Exception.Message)"
    }
}
...
function Start-InitialAccessDetection {
    try {
        ...
        if ($sysmonEvents) {
            $processEvents = $sysmonEvents | Where-Object { $_.Id -eq 1 }
            $fileEvents = $sysmonEvents | Where-Object { $_.Id -eq 11 }
            
            # T1566.001 - Spearphishing Attachment Detection
            foreach ($event in $processEvents) {
                # Log raw event first
                Write-RawEventLog -Event $event -LogSource "Sysmon"
                ...
                if ($parentImage -match "outlook\.exe|thunderbird\.exe|mailbird\.exe") {
                    if ($image -match "powershell\.exe|wscript\.exe|cscript\.exe|cmd\.exe") {
                        Write-SecurityEvent -LogType 'Main' -EventData @{
                            ...
                            EventDetails = 'Spearphishing attachment: Email client spawned script interpreter'
                            ProcessID = $processId
                            ProcessName = $image
                            ...
                            FilePath = ''
                            RegistryKey = ''
                            AdditionalContext = "Parent: $parentImage, ProcessGuid: $processGuid, Hashes: $hashes"
                        }
                    }
                }
                ...
                if ($parentImage -match "winword\.exe|excel\.exe|powerpnt\.exe") {
                    if ($image -match "powershell\.exe|wscript\.exe|cscript\.exe|cmd\.exe|dllhost\.exe") {
                        Write-SecurityEvent -LogType 'Main' -EventData @{
                            ...
                            MitreTechnique = 'T1566.001'
                            DetectionModule = 'InitialAccess'
                            EventDetails = 'Office macro execution detected'
                            ProcessID = $processId
                            ProcessName = $image
                            CommandLine = $commandLine
                            User = $user
                            ...
                            FilePath = ''
                            RegistryKey = ''
                            AdditionalContext = "Office App: $parentImage, ProcessGuid: $processGuid"
                        }
                    }
                }
                ...
                if ($parentImage -match "chrome\.exe|firefox\.exe|msedge\.exe|iexplore\.exe") {
                    if ($image -match "powershell\.exe|wscript\.exe|cscript\.exe|cmd\.exe") {
                        Write-SecurityEvent -LogType 'Main' -EventData @{
                            ...
                            DetectionModule = 'InitialAccess'
                            EventDetails = 'Drive-by compromise: Browser spawned script interpreter'
                            ProcessID = $processId
                            ProcessName = $image
                            CommandLine = $commandLine
                            User = $user
                ...
                if ($parentImage -match "w3wp\.exe|httpd\.exe|nginx\.exe|apache\.exe|iisexpress\.exe") {
                    if ($image -match "powershell\.exe|cmd\.exe|wscript\.exe|cscript\.exe") {
                        Write-SecurityEvent -LogType 'Main' -EventData @{
                            ...
                            DetectionModule = 'InitialAccess'
                            EventDetails = 'Web server exploit: Web server spawned system process'
                            ProcessID = $processId
                            ProcessName = $image
                            CommandLine = $commandLine
                            User = $user
                            ...
                            FilePath = ''
                            RegistryKey = ''
                            AdditionalContext = "WebServer: $parentImage"
                        }
                    }
                }
            }
            ...
            foreach ($event in $fileEvents) {
                ...
                # Monitor executable files in temp directories
                if ($targetFilename -match "\\Temp\\.*\.(exe|dll|scr|com|bat|cmd|ps1|vbs|js)$") {
                    Write-SecurityEvent -LogType 'Main' -EventData @{
                        Severity = 'WARNING'
                        MitreTechnique = 'T1566.001'
                        DetectionModule = 'InitialAccess'
                        EventDetails = 'Suspicious file created in temp directory'
                        ProcessID = $processId
                        ProcessName = $image
                        CommandLine = ''
                        User = ''
                        ...
                        RegistryKey = ''
                        AdditionalContext = "FileLocation: TempDirectory"
                    }
                }
                ...
                if ($targetFilename -match "\.(pdf|doc|docx|xls|xlsx|ppt|pptx)\.(exe|scr|com|bat|cmd|ps1|vbs)$") {
                    Write-SecurityEvent -LogType 'Main' -EventData @{
                        ...
                        MitreTechnique = 'T1566.001'
                        DetectionModule = 'InitialAccess'
                        EventDetails = 'File with double extension detected'
                        ProcessID = $processId
                        ProcessName = $image
                        CommandLine = ''
                        User = ''
        ...
        if ($securityEvents) {
            foreach ($event in $securityEvents) {
                # Log raw event first
                Write-RawEventLog -Event $event -LogSource "Security"
                ...
                # T1133 - External Remote Services (RDP logons)
                if ($event.Id -eq 4624) {
                    ...
                    if ($logonType -eq "10") { # RDP logon
                        ...
                        if ($ipAddress -and $ipAddress -notmatch "^192\.168\.|^10\.|^172\.(1[6-9]|2[0-9]|3[0-1])\.|^127\.|^169\.254\.") {
                            Write-SecurityEvent -LogType 'Main' -EventData @{
                                ...
                                DetectionModule = 'InitialAccess'
                                EventDetails = 'External RDP logon detected'
                                ProcessID = ''
                                ProcessName = ''
                                CommandLine = ''
                                User = $targetUserName
                                ...
                                FilePath = ''
                                RegistryKey = ''
                                AdditionalContext = "Workstation: $workstationName, LogonType: RDP"
                            }
                        }
                    }
                    
                    # T1078 - Valid Accounts (Off-hours logon)
                    $currentHour = (Get-Date).Hour
                    if ($currentHour -lt 6 -or $currentHour -gt 22) {
                        if ($logonType -in @("2", "3", "10")) {
                            Write-SecurityEvent -LogType 'Main' -EventData @{
                                Severity = 'WARNING'
                                MitreTechnique = 'T1078'
                                DetectionModule = 'InitialAccess'
                                EventDetails = 'Off-hours logon detected'
                                ProcessID = ''
                                ProcessName = ''
                                CommandLine = ''
                                User = $targetUserName
                                ...
                                FilePath = ''
                                RegistryKey = ''
                                AdditionalContext = "LogonType: $logonType, Time: $(Get-Date)"
                            }
                        }
                    }
                }
                
                # Monitor failed external logon attempts
                if ($event.Id -eq 4625) {
                    $targetUserName = $eventData["TargetUserName"]
                    $ipAddress = $eventData["IpAddress"]
                    ...
                    if ($ipAddress -and $ipAddress -notmatch "^192\.168\.|^10\.|^172\.(1[6-9]|2[0-9]|3[0-1])\.|^127\.|^169\.254\.") {
                        Write-SecurityEvent -LogType 'Main' -EventData @{
                            ...
                            MitreTechnique = 'T1133'
                            DetectionModule = 'InitialAccess'
                            EventDetails = 'Failed external logon attempt'
                            ProcessID = ''
                            ProcessName = ''
                            CommandLine = ''
                            User = $targetUserName
                            ...
                            FilePath = ''
                            RegistryKey = ''
                            AdditionalContext = "Reason: $failureReason"
                        }
                    }
                }
            }
...
function Start-ExecutionDetection {
    try {
        ...
        # Script interpreters and suspicious arguments
        $scriptInterpreters = @(
            "powershell\.exe",
            "pwsh\.exe", 
            "cmd\.exe",
...
function Start-CorrelationEngine {
    try {
        # Process events in correlation window
        $correlationWindow = (Get-Date).AddSeconds(-$Global:ServiceConfig.CorrelationWindow)
        
        # Read recent events from main log for correlation
        $mainLogPath = Join-Path $Global:ServiceConfig.LogBasePath $Global:ServiceConfig.MainLogFile
        
        if (Test-Path $mainLogPath) {
            $recentEvents = Import-Csv $mainLogPath | Where-Object {
                [DateTime]::Parse($_.Timestamp) -gt $correlationWindow
            }
            
            # Group events by user and time proximity
            $eventGroups = $recentEvents | Group-Object User | Where-Object { $_.Count -gt 2 }
            
            foreach ($group in $eventGroups) {
                $events = $group.Group | Sort-Object Timestamp
                $techniques = $events.MITRE_Technique | Select-Object -Unique
                
                # Look for attack chain patterns
                if ($techniques.Count -ge 3) {
                    $attackChain = $techniques -join "→"
                    $duration = ([DateTime]::Parse($events[-1].Timestamp) - [DateTime]::Parse($events[0].Timestamp)).TotalSeconds
                    $confidence = [math]::Min(95, ($techniques.Count * 15) + 25)
                    
                    # Calculate risk score
                    $riskScore = 0
                    foreach ($technique in $techniques) {
                        switch ($technique) {
                            { $_ -like "T1566*" } { $riskScore += 20 }  # Initial Access
                            { $_ -like "T1059*" } { $riskScore += 15 }  # Execution
                            { $_ -like "T1055*" } { $riskScore += 25 }  # Defense Evasion
                            { $_ -like "T1003*" } { $riskScore += 30 }  # Credential Access
                    ...
                    $killChainPhase = "Multiple Phases"
                    if ($techniques -contains "T1566.001") { $killChainPhase = "Initial Access → Post-Exploitation" }
                    
                    $correlationData = @{
                        CorrelationType = 'ATTACK_CHAIN'
                        Confidence = $confidence
                        AttackChain = $attackChain
                        RelatedEventIDs = ($events.Timestamp -join ";")
                        Duration = "$([int]$duration)s"
                        RiskScore = $riskScore
                        KillChainPhase = $killChainPhase
                    }
                    
                    Write-SecurityEvent -LogType 'Correlation' -EventData $correlationData
                }
            }
        }
    }
        ...
        Write-SecurityEvent -LogType 'Error' -EventData @{
            ...
            ErrorMessage = $_.Exception.Message
            StackTrace = $_.ScriptStackTrace
            RecoveryAction = 'Continue correlation analysis'
        }
    }
}
...
function Start-SecurityServiceLoop {
    try {
        ...
        Write-ServiceLog -Message "Security Logger Service started successfully" -Component "Service" -Status "STARTED"
            ...
            try {
                ...
                foreach ($module in $Global:ServiceConfig.EnabledModules) {
                    try {
                        ...
                        Write-ServiceLog -Message "Module $module failed: $($_.Exception.Message)" -Component $module -Status "ERROR"
                    }
                }
                
                # Run correlation engine every 10 loops (reduce overhead)
                if ($loopCount % 10 -eq 0) {
                    Start-CorrelationEngine
                }
                
                $loopCount++
                $loopDuration = (Get-Date) - $loopStart
                
                # Log service health every 100 loops
                if ($loopCount % 100 -eq 0) {
                    Write-ServiceLog -Message "Service health check - Loop: $loopCount, Duration: $($loopDuration.TotalSeconds)s" -Component "HealthCheck" -Status "INFO"
                }
                ...
                Write-ServiceLog -Message "Service loop error: $($_.Exception.Message)" -Component "ServiceLoop" -Status "ERROR"
                Start-Sleep -Seconds 30  # Longer sleep on error
            }
        }
        
        Write-ServiceLog -Message "Security Logger Service stopped" -Component "Service" -Status "STOPPED"
    }
    catch {
        Write-ServiceLog -Message "Critical service error: $($_.Exception.Message)" -Component "Service" -Status "CRITICAL"
        throw
    }
}
...
function Stop-SecurityService {
    $Global:ServiceRunning = $false
    Write-ServiceLog -Message "Service stop requested" -Component "Service" -Status "STOPPING"
}
...
Usage: .\Unified-SecurityLogger-Service_2.ps1 -Action <action>
...
Examples:
  .\Unified-SecurityLogger-Service_2.ps1 -Action Install
  .\Unified-SecurityLogger-Service_2.ps1 -Action Start
  .\Unified-SecurityLogger-Service_2.ps1 -Action Run

Output:
  UnifiedSecurityEvents.csv - Single CSV file containing:
    - Raw Windows event logs (Sysmon, Security, System)
    - Security detection alerts with MITRE ATT&CK mapping
    - USB threat detection events
    - OT/Industrial process monitoring
    - Service health and error logs
    - Comprehensive MITRE technique and category analysis
