...
# === Scheduled Task Monitoring (Legacy) ===
try {
...
# === Alternative Event Log Monitoring using Get-WinEvent ===
function Start-EventLogMonitoring {
    Write-Log "[INFO] Starting alternative event log monitoring..."
                ...
                try {
                    ...
                    foreach ($event in $events) {
                        ...
                        if ($scriptText -match 'Invoke|FromBase64String|Download|Reflection|bypass|hidden|encodedcommand|-enc|-nop') {
                            Write-Log "[SUSPICIOUS] ScriptBlock: PowerShell: $($scriptText.Substring(0,[Math]::Min(150,$scriptText.Length)))..."
                        }
                    }
                } catch {
                ...
                try {
                    ...
                    foreach ($event in $processEvents) {
                        try {
                            ...
                            if ($suspicious) {
                                Write-Log "[SYSMON] Suspicious Execution: $([System.IO.Path]::GetFileName($image)) CMD: $cmdLine"
                            }
                        } catch {}
                    }
                } catch {
                ...
                try {
                    ...
                    foreach ($event in $defenderEvents) {
                        try {
                            ...
                            Write-Log "[DEFENDER] Malware detection: Threat='$threatName' Path='$path'"
                        } catch {
                            Write-Log "[DEFENDER] Malware detection: EventID 1116"
                        }
                    }
                } catch {
...
# === File System Monitoring (Alternative approach) ===
try {
...
# Start alternative monitoring
Start-EventLogMonitoring
...
try {
    $registryJob = Start-Job -ScriptBlock {
        ...
        foreach ($key in $persistenceKeys) {
            try {
                $initialState[$key] = Get-ItemProperty -Path $key -ErrorAction SilentlyContinue | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name
            } catch {}
        }
        ...
        while ($true) {
            foreach ($key in $persistenceKeys) {
                try {
                    $currentState = Get-ItemProperty -Path $key -ErrorAction SilentlyContinue | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name
                    $newEntries = Compare-Object -ReferenceObject $initialState[$key] -DifferenceObject $currentState | Where-Object { $_.SideIndicator -eq '=>' }
                    
                    if ($newEntries) {
                        foreach ($entry in $newEntries) {
                            $value = (Get-ItemProperty -Path $key -Name $entry.InputObject -ErrorAction SilentlyContinue).$($entry.InputObject)
                            Write-Log "[PERSISTENCE] New registry entry: $key\$($entry.InputObject) = $value"
                        }
                        $initialState[$key] = $currentState
                    }
                } catch {}
            }
    ...
    Write-Log "[SUCCESS] Registry persistence monitoring job started (Job ID: $($registryJob.Id))."
} catch {
    Write-Log "[WARNING] Registry monitoring not available: $_"
}
